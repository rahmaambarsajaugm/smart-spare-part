/* =========================================================
   ...
   GOOGLE SHEET SHARED DATABASE VERSION
   Tampilan tetap sama; sumber data dipindah ke Google Sheet.
========================================================= */
...
async function loadFromGoogleSheet({ silent = false } = {}) {
    ...
    try {
        ...
        if (!payload || payload.success !== true || !Array.isArray(payload.data)) {
            throw new Error(payload?.message || "Format data Google Sheet tidak valid.");
        }
...
async function postToGoogleSheet(params) {
    // Dashboard dibuka langsung dari file://. Beberapa browser memblokir POST
    // lintas-origin ke Apps Script setelah redirect ContentService.
    // Karena GET API sudah terbukti dapat diakses dari komputer kantor,
    // aksi write dikirim sebagai query action ke doGet Apps Script.
...
function showDate() {
    ...
    if (!el) return;
    el.textContent = new Date().toLocaleDateString("id-ID", { day: "numeric", month: "long", year: "numeric" });
}
    ...
    tableBody.innerHTML = filtered.map((item, index) => `
        <tr>
            ...
            <td><div class="action-buttons">
                ...
                <button type="button" class="icon-action-btn delete-btn" onclick="deletePart(${item.id})" title="Hapus Spare Part"><i class="fa-solid fa-trash"></i></button>
            </div></td>
        </tr>`).join("");
...
async function updateStock(id, type) {
    ...
    try {
        const result = await postToGoogleSheet({
            ...
            type,
            qty,
            note: noteInput === null ? "" : noteInput.trim()
        });
        await loadFromGoogleSheet({ silent: true });
...
function showCategorySuggestions() {
    ...
    categorySuggestionBox.querySelectorAll("button").forEach(btn => btn.addEventListener("click", () => {
        partCategory.value = btn.textContent;
...
function exportBackup() {
    const payload = { app: "United Tractors Spare Part Monitoring - Google Sheet", version: 2, exported_at: new Date().toISOString(), spareParts, history };
...
function importBackup() {
    alert("Import JSON dinonaktifkan pada database bersama agar data Google Sheet tidak tertimpa tanpa sengaja. Tombol Backup tetap dapat digunakan.");
}

if (btnAdd) btnAdd.addEventListener("click", () => { if (requireConnection()) openModal(); });
...
window.addEventListener("DOMContentLoaded", async () => {
    ...
    setInterval(() => loadFromGoogleSheet({ silent: true }), AUTO_REFRESH_MS);
    console.log("Spare Part Monitoring terhubung ke Google Sheet shared database.");
});
